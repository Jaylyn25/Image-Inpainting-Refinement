...
from data.psf2otf import psf2otf
...
if __name__ == "__main__":
    img_path = '/content/drive/MyDrive/MSc_project/inpainting/pack/MEDFE_resize/data/0000.png'
    img = cv2.imread(img_path)
    S = L0Smoothing(img_path, param_lambda=0.05, param_kappa=1.5).run()
    S = np.squeeze(S)
    S = np.clip(S, 0, 1)
    S = S * 255
    S = S.astype(np.uint8)
    out = S
    #out = cv2.GaussianBlur(S,(9,9),0)
    #out = np.hstack((img, S))
