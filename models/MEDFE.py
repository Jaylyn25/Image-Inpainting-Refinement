...
class MEDFE(BaseModel):
    def __init__(self, opt):
        ...
        self.mask = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize1, opt.fineSize2)
        ...
        self.netEN, self.netDE, self.netMEDFE, self.stde_loss = networks.define_G(6, opt.output_nc, opt.ngf, opt.norm,
    ...
    def set_input(self, input_Gt, input_De, input_St, mask):

        self.Gt_DE = input_Gt.to(self.device)
        ...
        self.crop_y = random.randint(0, 256)
    ...
    def forward(self):
        ...
        input_ = torch.cat([self.input_DE, self.inv_ex_mask], 1)
        fake_p_1, fake_p_2, fake_p_3, fake_p_4, fake_p_5, fake_p_6 = self.netEN(torch.cat([self.input_DE, self.inv_ex_mask], 1))
    ...
    # input of RefineNet
    def get_inpainting_result(self, HE):
        input_image = (self.input_DE.data.cpu()+1)/2.0
        mask_image = (self.mask.data.cpu()+1)/2.0
        fake_image = (self.fake_out.data.cpu()+1)/2.0
        fake_image = F.interpolate(fake_image, size=(192,320), mode='bilinear')
        mask_image = F.interpolate(mask_image, size=(192,320), mode='bilinear')
        input_image = F.interpolate(input_image, size=(192,320), mode='bilinear')
        fake_image = fake_image.detach().cpu() * mask_image + input_image*(1-mask_image) #
        output = fake_image.detach().cpu().numpy()[0].transpose((1, 2, 0))*255
        real_gt = (self.Gt_DE.data.cpu()+1)/2.0
        real_gt = real_gt.detach().cpu().numpy()[0].transpose((1, 2, 0))*255
        # histogram equalization
        if HE:
            real_gt = exposure.equalize_hist(real_gt)
            real_gt = Image.fromarray((real_gt * 255).astype('uint8'))
            output = exposure.equalize_hist(output)
            output = Image.fromarray((output * 255).astype('uint8'))
        else:
            real_gt = Image.fromarray(real_gt.astype(np.uint8))
            output = Image.fromarray(output.astype(np.uint8))
        return output, real_gt
