...
class RefineNet(BaseModel):
    def __init__(self, opt):
        super(RefineNet, self).__init__(opt)
        ...
        self.input_DE = self.Tensor(opt.batchSize, opt.input_nc, opt.fineSize1, opt.fineSize2)
        self.input_ST = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize1, opt.fineSize2)
        self.Gt_Local = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize1, opt.fineSize2)
        self.Gt_DE = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize1, opt.fineSize2)
        self.Gt_ST = self.Tensor(opt.batchSize, opt.output_nc, opt.fineSize1, opt.fineSize2)
        self.mask_global = self.Tensor(opt.batchSize, 1, opt.fineSize1, opt.fineSize2)
        self.model_names = []
        ...
        self.netEN, self.netDE, self.stde_loss = networks.define_R(opt.input_nc, opt.output_nc, opt.ngf,
                                                                                 opt.norm,
                                                                                 opt.use_dropout, opt.init_type,
                                                                                 self.gpu_ids,
                                                                                 opt.init_gain)
        # define and initial generator
        # self.netRefine, self.stde_loss = networks.define_R(opt.init_type,self.gpu_ids,opt.init_gain)
        self.model_names = ['EN', 'DE']

        if self.isTrain:
            # define and initial discriminator
            self.netD = networks.define_D(3, opt.ndf,
                                          opt.n_layers_D, opt.norm, opt.init_type, self.gpu_ids, opt.init_gain, 3)
        ...
        if self.isTrain:
            ...
            #self.optimizer_Refine = torch.optim.Adam(self.netRefine.parameters(),
                                                  #  lr=opt.lr, betas=(opt.beta1, 0.999))
            ...
            #networks.print_network(self.netRefine)
            if self.isTrain:
        ...
        if self.isTrain:
            if opt.continue_train:
                ...
                #self.load_networks(self.netRefine, 'Refine', opt.which_epoch)
    ...
    def set_input(self, input_Gt, input_De):

        self.Gt_DE = input_Gt.to(self.device) #ground truth
        #self.Gt_ST = input_St.to(self.device)
        self.input_DE = input_De.to(self.device)  #noise image
        self.Gt_Local = input_Gt.to(self.device)
        # define local area which send to the local discriminator
        self.crop_x = random.randint(0, 128) #191
        self.crop_y = random.randint(0, 256) #191
        ...
        #self.stde_loss[0].set_target(self.Gt_DE, self.Gt_DE)
        # apply masks, RGB - 012
    ...
    def forward(self):
        # print("input:",self.input_DE.size())
        #self.fake_out = self.netRefine(self.input_DE)
        fake_p_1, fake_p_2, fake_p_3, fake_p_4, fake_p_5, fake_p_6 = self.netEN(self.input_DE)
        De_in = [fake_p_1, fake_p_2, fake_p_3, fake_p_4, fake_p_5, fake_p_6]
        self.fake_out = self.netDE(De_in[0], De_in[1], De_in[2], De_in[3], De_in[4], De_in[5])
        # print("output:",self.fake_out.size())
    ...
    def backward_D(self):
        ...
        fake_local = self.fake_out[:, :, self.crop_x:self.crop_x + 32, self.crop_y:self.crop_y + 32]
    ...
    def optimize_parameters(self):
        ...
        #self.set_requires_grad(self.netRefine, False)
