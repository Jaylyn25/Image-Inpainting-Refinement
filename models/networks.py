...
# reconstruction model for refinement
def define_R(input_nc, output_nc, ngf,  norm='batch', use_dropout=False, init_type='normal', gpu_ids=[], init_gain=0.02):
    norm_layer = get_norm_layer(norm_type=norm)
    ...
    return init_net(netEN, init_type, init_gain, gpu_ids), init_net(netDE, init_type, init_gain, gpu_ids), stde_list


def define_D(input_nc, ndf, n_layers_D=3, norm='batch',  init_type='normal', gpu_ids=[], init_gain=0.02, kw=4):
    netD = None
    norm_layer = get_norm_layer(norm_type=norm)
    netD = NLayerDiscriminator(input_nc, kw, ndf, n_layers=n_layers_D, norm_layer=norm_layer, use_sigmoid=False)
...
class PCblock(nn.Module):
    ...
    def forward(self, input, mask):
        out = self.pc_block(input, mask) #<--PCconv
        out = self.loss(out) #<--InnerCos
        return out
