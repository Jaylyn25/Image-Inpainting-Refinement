...
if __name__ == "__main__":
    ...
    # define the dataset
    dataset = DataProcess_I(opt.gt_root,opt.synth_root,opt,opt.isTrain)
    iterator_train = (data.DataLoader(dataset, batch_size=opt.batchSize, shuffle=True, num_workers=opt.num_workers))
    # Create model
    model = create_model(opt)
    model.netEN.module.load_state_dict(torch.load("./pretrained_models/place2/EN.pkl"))
    model.netDE.module.load_state_dict(torch.load("./pretrained_models/place2/DE.pkl"))
    model.netMEDFE.module.load_state_dict(torch.load("./pretrained_models/place2/MEDFE.pkl"))
    # model.load_networks_I(6) # epoch 6 pre-trained
    ...
    for epoch in range (opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        epoch_iter = 0
        for gt, detail, structure, mask in iterator_train:
            iter_start_time = time.time()
            total_steps += opt.batchSize
            epoch_iter += opt.batchSize
            model.set_input(gt, detail, structure, mask)
        ...
        if epoch % opt.save_epoch_freq == 0:
            print('saving the model at the end of epoch %d, iters %d' %
                  (epoch, total_steps))
            model.save_inpainting_networks(epoch)
