...
from models.models import create_model_R
import torchvision
...
if __name__ == "__main__":
    ...
    # define the dataset
    #dataset = DataProcess_R(opt.gt_root,opt.synth_root,opt,opt.isTrain)
    ...
    # model.netEN.module.load_state_dict(torch.load("/content/drive/MyDrive/MSc_project/refinement/model-checkpoints/Refine_Net/30_net_EN.pth"))
    # model.netDE.module.load_state_dict(torch.load("/content/drive/MyDrive/MSc_project/refinement/model-checkpoints/Refine_Net/30_net_DE.pth"))
    model_R.load_networks_R(30) # epoch 120 pre-trained
    ...
    for epoch in range (opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        epoch_iter = 0
        for gt, detail in iterator_train:
            iter_start_time = time.time()
            total_steps += opt.batchSize
            epoch_iter += opt.batchSize
            model.set_input(gt, detail)
