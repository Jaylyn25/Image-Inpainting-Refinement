...
def gussin(v):
    outk = []
    v = v
    for i in range(int(fineSize1/8)): #32
        for k in range(int(fineSize2/8)): #32

            out = []
            for x in range(int(fineSize1/8)): #32
                row = []
                for y in range(int(fineSize2/8)): #32
                    cord_x = i
                    cord_y = k
    ...
    f = out.sum(-1).sum(-1)
    q = []
    for i in range(int(fineSize1/8)*int(fineSize2/8)): #1024
        g = out[i] / f[i]
        q.append(g)
    out = np.array(q)
